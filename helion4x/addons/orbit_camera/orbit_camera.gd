class_name OrbitCamera extends Camera
...
func _process_movement(delta):
	...
	if Input.is_action_pressed("anchor_forward"):
		direction.x += 1
	if Input.is_action_pressed("anchor_back"):
		direction.x -= 1
	if Input.is_action_pressed("anchor_left"):
		direction.y += 1
	if Input.is_action_pressed("anchor_right"):
		direction.y -= 1
